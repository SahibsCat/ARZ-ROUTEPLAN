...
// key already configured) to turn on the live marker map. Every other part
// of this workspace works with or without it - the map panel just falls
// back to a plain "open in Google Maps" link until it's set.
...
function RouteMapPanel({ route, selectedOrderId, onSelectOrder }) {
  ...
  // hooks) - but it only actually injects Google's script tag once a
  // non-empty key is passed, so an unset key never fires a network request.
  ...
  return (
    ...
    >
      {stops.map((order, idx) => {
        const letter = sequenceLabel(idx);
        ...
        return (
          ...
          >
            <button
              ...
              title={`${letter} — ${order.customer_name || order.order_id}`}
            >
              {letter}
            </button>
          </OverlayView>
